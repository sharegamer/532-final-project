...
class TFIDFRecommender:
    """Content-based recommender using TF-IDF similarity."""

    def __init__(self, tfidf_matrix, news_ids):
        """
        Initialize TF-IDF recommender.

        Args:
            tfidf_matrix: TF-IDF feature matrix (scipy sparse matrix or numpy array)
            news_ids: List/array of news IDs corresponding to rows in tfidf_matrix
        """
    ...
    def compute_similarity_matrix(self):
        ...
        This is memory intensive but speeds up recommendations.
        """
    ...
    def get_similar_news(self, news_id, top_k=10):
        """
        Get most similar news articles to a given news article.
        ...
        # Compute similarity if not precomputed
        if self.similarity_matrix is None:
            news_vector = self.tfidf_matrix[idx:idx+1]
    ...
    def recommend_for_user(self, user_history, candidate_news, top_k=10):
        """
        Recommend news for a user based on their reading history.
        ...
        # Compute user profile as average of history TF-IDF vectors
        user_profile = np.asarray(self.tfidf_matrix[history_indices].mean(axis=0))

        # Get indices of candidate news
    ...
    def get_user_profile_vector(self, user_history):
        """
        Get user profile vector based on reading history.

        Args:
            user_history: List of news IDs

        Returns:
            User profile vector (TF-IDF averaged)
        """
        valid_history = [nid for nid in user_history if nid in self.news_id_to_idx]
    ...
    def save_model(self, path):
        ...
        with open(f'{path}/tfidf_recommender.pkl', 'wb') as f:
            pickle.dump({
    ...
    def load_model(path):
        ...
        with open(f'{path}/tfidf_recommender.pkl', 'rb') as f:
            data = pickle.load(f)

        recommender = TFIDFRecommender(data['tfidf_matrix'], data['news_ids'])
